    ...
    ui.horizontal(|ui| {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            match settings_state.selected_section {
                ...
                SettingsSection::AccountStorage => {
                    section_frame.show(ui, |ui: &mut egui::Ui| {
                            ...
                            ui.indent("tiling_options_indent", |ui| {
                                ...
                                setting_row(ui, "target_display", |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        let current_label = match &config.tiling_target_monitor {
                                            ...
                                            ram_core::models::MonitorTarget::Index(i) => monitors
                                                .get(*i)
                                                .map(|m| m.name.clone())
                                ...
                                setting_row(ui, "grid_layout", |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        let current_layout_label = match &config.tiling_layout_mode {
                                            ...
                                            ram_core::models::TilingLayoutMode::CustomGrid { .. } => "Custom Grid (Cols × Rows)",
                                        ...
                                        egui::ComboBox::from_id_salt("tiling_layout_mode_combo")
                                            ...
                                            .show_ui(ui, |ui| {
                                                if ui.selectable_label(matches!(config.tiling_layout_mode, ram_core::models::TilingLayoutMode::Auto), "Auto Grid (Square-like)").clicked() {
                                                    config.tiling_layout_mode = ram_core::models::TilingLayoutMode::Auto;
                                                }
                                                if ui.selectable_label(matches!(config.tiling_layout_mode, ram_core::models::TilingLayoutMode::FixedColumns(_)), "Fixed Columns").clicked() {
                                                    config.tiling_layout_mode = ram_core::models::TilingLayoutMode::FixedColumns(config.tiling_custom_cols);
                                                }
                                                if ui.selectable_label(matches!(config.tiling_layout_mode, ram_core::models::TilingLayoutMode::FixedRows(_)), "Fixed Rows").clicked() {
                                                    config.tiling_layout_mode = ram_core::models::TilingLayoutMode::FixedRows(config.tiling_custom_rows);
                                                }
                                                if ui.selectable_label(matches!(config.tiling_layout_mode, ram_core::models::TilingLayoutMode::CustomGrid { .. }), "Custom Grid (Cols × Rows)").clicked() {
                                                    config.tiling_layout_mode = ram_core::models::TilingLayoutMode::CustomGrid { cols: config.tiling_custom_cols, rows: config.tiling_custom_rows };
                                                }
                                                if ui.selectable_label(matches!(config.tiling_layout_mode, ram_core::models::TilingLayoutMode::SideBySide), "Side-by-Side (1 Row)").clicked() {
                                                    config.tiling_layout_mode = ram_core::models::TilingLayoutMode::SideBySide;
                                                }
                                                if ui.selectable_label(matches!(config.tiling_layout_mode, ram_core::models::TilingLayoutMode::Stacked), "Stacked (1 Column)").clicked() {
                                    ...
                                    ram_core::models::TilingLayoutMode::FixedColumns(cols) => {
                                        setting_row(ui, "layout_dimensions", |ui| {
                                            ui.horizontal(|ui| {
                                                ...
                                                if ui.add(egui::DragValue::new(&mut c).range(1..=12).speed(0.1)).changed() {
                                    ...
                                    ram_core::models::TilingLayoutMode::FixedRows(rows) => {
                                        setting_row(ui, "layout_rows", |ui| {
                                            ui.horizontal(|ui| {
                                                ...
                                                if ui.add(egui::DragValue::new(&mut r).range(1..=12).speed(0.1)).changed() {
                                ...
                                setting_row(ui, "window_padding", |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        if ui.add(egui::DragValue::new(&mut pad).range(0..=50).suffix(" px").speed(0.2)).changed() {
                                ...
                                ui.add_space(8.0);
                                if ui.button("Tile Windows Now").on_hover_text("Immediately arrange all open Roblox windows using these settings.").clicked() {
                ...
                SettingsSection::Privacy => {
                    section_frame.show(ui, |ui: &mut egui::Ui| {
                        ...
                        setting_row(ui, "privacy_mode", |ui| {
                            ui.checkbox(&mut config.privacy_mode, "Clean before launch");
                        });
                        ui.vertical(|ui| {
                            ui.indent("privacy_cleanup_options", |ui| {
                                ui.add_enabled_ui(config.privacy_mode, |ui| {
                                    setting_row(ui, "privacy_cookies", |ui| {
                                        ui.checkbox(&mut config.privacy_clean_cookies, "Clean cookies");
                                    });
                                    setting_row(ui, "privacy_local_storage", |ui| {
                                        ui.checkbox(&mut config.privacy_clean_local_storage, "Clean cookies and LocalStorage");
                                    });
                                    setting_row(ui, "privacy_full_profile", |ui| {
                                        ui.checkbox(&mut config.privacy_clean_full_profile, "Clean full Roblox cache/profile");
                                    });
                                    setting_row(ui, "privacy_on_exit", |ui| {
                                        ui.checkbox(&mut config.privacy_clean_on_exit, "Clean selected privacy data on exit");
                                    });
                                });
                            });
                        });
                        ...
                        setting_row(ui, "mac_rotation", |ui| {
                            ui.checkbox(&mut config.mac_rotation_enabled, "Enable MAC address rotation");
                        });
                        if config.mac_rotation_enabled {
                            ui.vertical(|ui| {
                                ui.indent("mac_rotation_options", |ui| {
                                    setting_row(ui, "mac_preserve_oui", |ui| {
                                        ui.checkbox(&mut config.mac_preserve_oui, "Keep this PC's adapter OUI");
                                    });
                                    if !config.mac_preserve_oui {
                                        setting_row(ui, "mac_alternate_oui", |ui| {
                                            egui::ComboBox::from_id_salt("mac_alternate_oui")
                                                ...
                                                .show_ui(ui, |ui| {
                                                    ui.selectable_value(&mut config.mac_alternate_oui, "00:1B:21".to_string(), "Intel (00:1B:21)");
                                                    ui.selectable_value(&mut config.mac_alternate_oui, "00:E0:4C".to_string(), "Realtek (00:E0:4C)");
                                                    ui.selectable_value(&mut config.mac_alternate_oui, "3C:52:82".to_string(), "Microsoft (3C:52:82)");
                                                });
                                        });
                                    }
                ...
                SettingsSection::AppData => {
                    section_frame.show(ui, |ui: &mut egui::Ui| {
                        ...
                        if ui.button("Clean orphaned data").on_hover_text("Remove browse-as profiles for accounts no longer in RM").clicked() {
                        ...
                        if config.developer_options {
                            ...
                            if ui.button("Clear application caches").on_hover_text("Remove avatars, thumbnails, metadata, and inventory caches. Accounts and assets are kept.").clicked() {
                ...
                SettingsSection::Security => {
                    section_frame.show(ui, |ui: &mut egui::Ui| {
                        ...
                        if !settings_state.new_password_input.is_empty()
                            ...
                            && !passwords_match
                        {
                            ui.colored_label(theme.danger, "Passwords do not match.");
                        }
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if ui.add_enabled(passwords_match, egui::Button::new(label)).clicked() {
                                ...
                                action = Some(SettingsAction::ChangePassword { new_password: new_pw });
                            }
